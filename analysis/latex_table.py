...
def make_latex_table(releasename='February2021', savename='selfcal_summary'):
    if datetime.datetime.today() > datetime.datetime(year=2021, month=1, day=10):
        result = requests.get(f'https://data.rc.ufl.edu/secure/adamginsburg/ALMA-IMF/{releasename}Release/tables/metadata_sc.ecsv',
    ...
    tbl = table.join(Table.read(f'{releasename}_metadata_sc.ecsv'), bp_tbl, keys=('region', 'band'))
    ...
    fwtbl.write(f"../datapaper/{savename}.tex", formats=formats,
...
if __name__ == "__main__":
    ...
    feb_sc = make_latex_table(releasename='February2021', savename='selfcal_summary')
    jun_sc = make_latex_table(releasename='June2021', savename='selfcal_summary_June2021')
