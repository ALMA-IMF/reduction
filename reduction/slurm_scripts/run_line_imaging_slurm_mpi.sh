...
echo srun --mpi=pmix_v3 xvfb-run -d ${MPICASA} -n 8 ${CASA} --nogui --nologger --logfile=${LOGFILENAME} -c "execfile('$SCRIPT_DIR/line_imaging.py')"
srun --mpi=pmix_v3 xvfb-run -d ${MPICASA} -n 8 ${CASA} --nogui --nologger --logfile=${LOGFILENAME} -c "execfile('$SCRIPT_DIR/line_imaging.py')"
